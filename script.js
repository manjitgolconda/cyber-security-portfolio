...
(function () {
  ...
  // =========================================================================
  // INTERACTIVE BATCAVE CLI TERMINAL
  // =========================================================================
  ...
  const terminalCommands = {
    help: () => `
COMMAND DIRECTORY:
  <span class="out-crimson">help</span>             Display all available commands
  <span class="out-info">whoami</span>           Print operator identity, specialization & contact
  <span class="out-info">skills</span>           Inspect technical skillset & security toolset
  <span class="out-info">projects</span>         List active operations & featured repositories
  <span class="out-info">education</span>        View academic record & university coursework
  <span class="out-info">certs</span>            Display verified certifications & workshops
  <span class="out-info">batsignal</span>        Toggle Bat-Signal spotlight overlay
  <span class="out-info">scan</span>             Execute deep VAPT vulnerability network scan
  <span class="out-info">decrypt &lt;name&gt;</span>   Decrypt operation dossier (e.g. 'decrypt saferoute')
  <span class="out-info">matrix</span>           Stream encrypted binary rain
  <span class="out-info">date</span>             Display current UTC timestamp
  <span class="out-info">audio</span>            Toggle tactical sound effects
  <span class="out-info">contact</span>          Open encrypted dispatch channel
  <span class="out-info">clear</span>            Clear terminal screen
`,
    whoami: () => `
OPERATOR:           <span class="out-crimson">MANJIT GOLCONDA</span>
TITLE:              <span class="out-warn">Cyber Security Analyst | Ethical Hacker | VAPT Specialist</span>
NATIONALITY / LOC:  Indian / Hyderabad, India
ACADEMICS:          B.Tech CSE (Cyber Security), Sphoorthy Engineering College (2023–2027)
CONTACT:            Mobile: +91 9063364603 | Email: manjitgolconda@gmail.com
GITHUB / LINKEDIN:  github.com/manjitgolconda | linkedin.com/in/manjit-golconda
STATUS:             ACTIVE & CONDUCTING PRACTICAL CYBER RESEARCH
`,
    skills: () => `
TACTICAL ARSENAL BREAKDOWN:
  [+] <span class="out-crimson">CYBERSECURITY</span>: VAPT, Network Security, Web Security, Ethical Hacking, Recon, Vuln Assessment
  [+] <span class="out-info">TOOLS</span>:         Kali Linux, Nmap, OpenVAS, OWASP ZAP, SQLMap, Wireshark
  [+] <span class="out-info">PROGRAMMING</span>:   Python, JavaScript, Java, HTML, CSS
  [+] <span class="out-warn">DEV & INFRA</span>:   React.js, Tailwind CSS, Node.js, Express, Leaflet, REST APIs, Git/GitHub, Vercel
`,
    projects: () => `
ACTIVE OPERATIONS & REPOSITORIES:
  1. <span class="out-crimson">SAFE ROUTE HYDERABAD</span> - Civic AI Safety Intelligence Navigation (Leaflet/OSRM)
  2. <span class="out-info">MSME CONNECT</span>         - Hyperlocal Verified B2B Marketplace (React/Node/Express/JWT)
  3. <span class="out-info">INTERNAL VAPT LAB</span>    - Isolated Network Mapping & Exploitation Analysis Suite
  4. <span class="out-success">MALICIOUS EMAIL OSINT</span>- Threat Intelligence & Phishing Forensics (Maltego/Python)
Type '<span class="out-crimson">decrypt saferoute</span>' to inspect operation schematics.
`,
    education: () => `
ACADEMIC RECORD:
  [+] DEGREE:         B.Tech in Computer Science Engineering — Cyber Security
  [+] INSTITUTION:    Sphoorthy Engineering College, Hyderabad (2023 – 2027)
  [+] COURSEWORK:     Network Security, Cybersecurity, Computer Networks, Web Tech, OS, DBMS, Software Engg
  [+] KEY INTERESTS:  Ethical Hacking, VAPT, Network Security, Web Security, Linux, Secure Apps, NLP
`,
    certs: () => `
VERIFIED CERTIFICATIONS & WORKSHOPS:
  [1] Natural Language Processing for Developers — Infosys / Springboard (Aug 2026)
  [2] Cybersecurity — Skill India Digital Hub / Tech Mahindra Foundation (Aug 2026)
  [3] Introduction to Cybersecurity — Cisco Networking Academy (Aug 2025)
  [4] Programming Foundations with Python — CCBP 4.0 Academy (2024)
  [5] NxtCode-7 Under 7 Challenge — Certificate of Participation (Oct 2024)
  [6] AWS Mega Workshop — Cloud Computing (Jun 2024)
  [7] Ethical Hacking Mega Workshop — Offensive Security (Oct 2023)
  [8] Malicious Email Investigation using Maltego — Threat Forensics (Oct 2023)
  [9] Cyber Oath — Building India's Cyber Army (Sep 2023)
  [10] Generative AI Mega Workshop — Large Language Models (Aug 2023)
`,
    batsignal: () => {
      if (btnToggleBatSignal) btnToggleBatSignal.click();
      return `<span class="out-crimson">[!] BAT-SIGNAL OVERLAY STATE TOGGLED.</span>`;
    },
    audio: () => {
      if (btnToggleAudio) btnToggleAudio.click();
      return `<span class="out-info">[!] AUDIO SUBSYSTEM STATE TOGGLED.</span>`;
    },
    scan: () => `
<span class="out-info">[&gt;] INITIALIZING DEEP VAPT VULNERABILITY SCAN ACROSS LAB INTERFACES...</span>
  [OK] Probing target endpoints with Nmap SYN scan...
  [OK] Scanning web attack surfaces for OWASP Top 10 vulnerabilities...
  [OK] Auditing SQL injection parameters with SQLMap heuristics...
  [OK] Inspecting protocol integrity for ARP/DNS anomalies...
  <span class="out-success">[+] STATUS: SCAN COMPLETE. ZERO CRITICAL EXPLOITATION SURFACE DETECTED.</span>
`,
    date: () => `CURRENT TIME: <span class="out-info">${new Date().toUTCString()}</span>`,
    contact: () => {
      const contactEl = document.getElementById('contact');
      if (contactEl) contactEl.scrollIntoView({ behavior: 'smooth' });
      return `<span class="out-crimson">[&gt;] NAVIGATING TO ENCRYPTED DISPATCH MODULE...</span>`;
    },
    matrix: () => {
      let matrixLines = '';
      for (let i = 0; i < 6; i++) {
        ...
        matrixLines += `<span class="out-success">${line}</span>\n`;
      }
      return matrixLines + `<span class="out-crimson">[CIPHER STREAM DECODED]</span>`;
    },
    sudo: (args) => `<span class="out-error">ACCESS GRANTED: Operator Manjit Golconda root session authorized.</span>`,
    clear: () => {
  ...
  window.executeCLICommand = function (cmdStr) {
    if (!cmdStr || !terminalOutput) return;
    const trimmed = cmdStr.trim();
    const parts = trimmed.split(' ');
    const mainCmd = parts[0].toLowerCase();
    const args = parts.slice(1).join(' ');

    const promptLine = `<div style="margin-top: 8px;"><span class="out-crimson">manjit@hyd-sec-core:~$</span> ${trimmed}</div>`;
    ...
    if (mainCmd === 'decrypt') {
      if (!args) {
        terminalOutput.innerHTML += `<div class="out-warn">Usage: decrypt &lt;saferoute | msme | pentestlab | maltego&gt;</div>`;
      } else {
        const targetProj = args.toLowerCase();
        if (projectData[targetProj]) {
          openProjectModal(targetProj);
          terminalOutput.innerHTML += `<div class="out-success">[+] Operation '${targetProj}' schematic decrypted and displayed.</div>`;
        } else {
          terminalOutput.innerHTML += `<div class="out-error">[-] Operation '${targetProj}' not recognized in classified archives. Available: saferoute, msme, pentestlab, maltego</div>`;
        }
      }
    } else if (terminalCommands[mainCmd]) {
      const res = terminalCommands[mainCmd](args);
      if (res) terminalOutput.innerHTML += `<div>${res}</div>`;
      soundSuccess();
    } else {
      terminalOutput.innerHTML += `<div class="out-error">[-] Command not found: '${mainCmd}'. Type '<span class="out-crimson">help</span>' for a list of commands.</div>`;
      soundError();
    }
